...
CG_INLINE CGFloat kBottomViewScale() {
    return .7;
}

CG_INLINE CGFloat kTopViewScale() {
    return .5;
}

CG_INLINE CGFloat kAnimateDuration() {
    return .1;
}

@protocol CustomCameraToolViewDelegate <NSObject>
...
@interface CustomCameraToolView : UIView <CAAnimationDelegate, UIGestureRecognizerDelegate>
{
    struct {
...
@property (nonatomic, weak) id<CustomCameraToolViewDelegate> delegate;
...
@implementation CustomCameraToolView
...
- (void)setDelegate:(id<CustomCameraToolViewDelegate>)delegate
{
    _delegate = delegate;
...
@interface CustomCameraViewController ()<CustomCameraToolViewDelegate, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate>{
    //拖拽手势开始的录制
    BOOL _dragStart;
    BOOL _layoutOK;
}

@property (nonatomic, strong) CustomCameraToolView *toolView;
//拍照录视频相关
...
- (void)setupUI
{
    ...
    self.toolView = [[CustomCameraToolView alloc] init];
